...
export default function Orders_Sponsors({isSpoofing = false, sponsorSpoofID = ''}) {
    ...
    useEffect(() => {
      const fetchOrders = async () => {
        try {
          ...
          const response = await fetch(`/api/sponsor/get_orders?org_ID=${orgID}`, requestOptions);
    ...
    console.log(`Accepting ${order.name}'s order`);
    ...
    fetch('api/sponsor/post_accept_order', requestOptions)
          ...
          <div>
            ...
            {orders.map((order) => (
              <Card key={order.order_id} style={{ marginBottom: '16px' }}>
                <CardContent>
                  <Typography variant="h6">Driver: {order.name}</Typography>
                  <Typography variant="body1">Order #: {order.order_id}</Typography>
