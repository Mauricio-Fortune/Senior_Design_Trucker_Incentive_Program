...
export default async function handler(req, res) {
    ...
    try {
        ...
        const checkQuery = 'SELECT * FROM User_Org WHERE user_ID = ? AND org_ID = ? AND active_User = 0';
        ...
        if (existingEntries.length > 0) {
            ...
            // If the user is already associated but not current, set active_User to 1
            const updateQuery = 'UPDATE User_Org SET active_User = 1, app_Status = "ACCEPTED" WHERE user_ID = ? AND org_ID = ? AND active_User = 0';
            ...
            const insertQuery = 'INSERT INTO User_Org (user_ID, org_ID, app_Status, active_User) VALUES (?, ?, "ACCEPTED", 1)';
